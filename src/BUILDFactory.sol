...
contract BUILDFactory is IBUILDFactory, ITypeAndVersion, ManagedAccessControl {
    ...
    mapping(uint256 seasonId => uint256 unlockStartsAt)
        private s_seasonUnlockStartTimes;

    /// @notice The project season configs
    mapping(address token => mapping(uint256 seasonId => ProjectSeasonConfig config))
        private s_projectSeasonConfigs;
    ...
    /// starts.
    mapping(address token => mapping(uint256 seasonId => uint256 amount))
        private s_refundableAmounts;
    ...
    function removeProjects(
        ...
        for (uint256 i = 0; i < tokensLength; ++i) {
            ...
            // @audit critical Deleting the project config struct does not clean up associated state in other mappings like `s_projectSeasonConfigs`, `s_refundableAmounts`, `s_tokenAmounts`, or `s_withdrawals`. If a new project is added with the same token address, it will inherit the old project's accounting state, leading to potential fund theft or locking.
    ...
    function _requireRegisteredClaim(address token) internal view {
    ...
    function _setUnlockConfigMaxValues(UnlockMaxConfigs memory config) private {
    ...
    /// @inheritdoc IBUILDFactory
    function getUnlockConfigMaxValues()
        external
        view
        override
        returns (UnlockMaxConfigs memory)
    {
    ...
    function setSeasonUnlockStartTime(
        ...
        bool hasUnlockStarted = currentUnlockStartTime != 0 &&
            currentUnlockStartTime <= block.timestamp;
    ...
    function _setProjectSeasonConfig(
        ...
        ProjectSeasonConfig storage currentConfig = s_projectSeasonConfigs[
            params.token
        ][params.seasonId];
        ...
        if (
            ...
            config.unlockDuration > unlockMaxConfigs.maxUnlockDuration
        ) {
            revert InvalidUnlockDuration(
                params.seasonId,
        ...
        if (
            ...
            (config.baseTokenClaimBps == PERCENTAGE_BASIS_POINTS_DENOMINATOR &&
                config.unlockDuration > 1)
        ) {
            revert InvalidBaseTokenClaimBps(
                params.seasonId,
                config.baseTokenClaimBps,
                config.unlockDuration
            );
        }
        ...
        if (
            ...
            revert InvalidEarlyVestRatios(
                config.earlyVestRatioMinBps,
                config.earlyVestRatioMaxBps
            );
        }
    ...
    function _setSeasonTokenAmount(
        ...
        uint256 totalAllocatedAmountBefore = tokenAmounts
            .totalAllocatedToAllSeasons;
        uint256 maxAvailable = _calcMaxAvailableForWithdrawalOrNewSeason(
            tokenAmounts
        );
    ...
    function getProjectSeasonConfig(
        ...
        view
        override
        returns (ProjectSeasonConfig memory, uint256 seasonUnlockStartTime)
    {
        return (
            s_projectSeasonConfigs[token][seasonId],
    ...
    function addTotalDeposited(
        ...
        emit ProjectTotalDepositedIncreased(
            token,
            msg.sender,
            amount,
            newTotalDeposited
    ...
    function reduceRefundableAmount(
        ...
        emit ProjectSeasonRefundableAmountReduced(
            ...
            seasonId,
            amount,
            currentRefundableAmount - amount
        );
    }
    ...
    function startRefund(
        ...
        ProjectSeasonConfig storage config = s_projectSeasonConfigs[token][
            seasonId
        ];
        ...
        uint256 totalLoyaltyRefundEligible = _getTotalLoyaltyRefundEligible(
            token,
            seasonId,
            config.tokenAmount,
            refundEligible
        );
        s_tokenAmounts[token].totalRefunded +=
            refundEligible +
            totalLoyaltyRefundEligible;
    ...
    function _getTotalLoyaltyRefundEligible(
        ...
        IBUILDClaim.GlobalState memory globalState = s_projects[token]
            .claim
            .getGlobalState(seasonId);
        if (globalState.totalLoyalty == 0) {
            return 0;
        }
        // By definition tokenAmount is always greater than or equal to totalLoyaltyIneligible
        uint256 totalLoyaltyEligible = tokenAmount -
        ...
        return
            globalState.totalLoyalty.mulWadDown(refundEligible).divWadDown(
                totalLoyaltyEligible
            );
    }
    ...
    function getRefundableAmount(
        ...
        ProjectSeasonConfig memory config = s_projectSeasonConfigs[token][
            seasonId
        ];
        uint256 refundEligible = s_refundableAmounts[token][seasonId];
        uint256 totalLoyaltyRefundEligible = _getTotalLoyaltyRefundEligible(
        ...
        return
            s_refundableAmounts[token][seasonId] + totalLoyaltyRefundEligible;
    }
    ...
    function scheduleWithdraw(
        ...
        s_withdrawals[token] = Withdrawal({
            recipient: recipient,
            amount: amount
        });
    ...
    function executeWithdraw(
        ...
        emit WithdrawalExecuted(
            ...
            withdrawal.recipient,
            withdrawal.amount,
            s_tokenAmounts[token].totalWithdrawn
        );
    ...
    function _validateNewWithdrawal(
        address token,
        uint256 amount
    ) private view {
    ...
    function _requireClaimNotPaused(address token) internal view virtual {
        if (isClaimContractPaused(token)) {
    ...
    /// @param token address of the project token
    function _ensureProjectExists(address token) internal view {
    ...
    function _setDelegateRegistry(IDelegateRegistry delegateRegistry) internal {
