...
contract BUILDClaim is IBUILDClaim, ITypeAndVersion, ReentrancyGuard {
    ...
    mapping(address user => mapping(uint256 seasonId => UserState))
        private s_userStates;
    ...
    function deposit(
        ...
        uint256 totalDeposited = i_factory.addTotalDeposited(
            address(i_token),
            amount
        );
    ...
    function getUserState(
        ...
        for (uint256 i; i < count; ++i) {
            states[i] = s_userStates[usersAndSeasonIds[i].user][
                usersAndSeasonIds[i].seasonId
            ];
        }
        return states;
    }
    ...
    function getCurrentClaimValues(
        ...
        UnlockState memory unlockState = _getUnlockState(
            unlockStartsAt,
            config.unlockDelay,
    ...
    function _verifyMerkleProof(
        ...
        bytes32 leaf = keccak256(
            bytes.concat(
                keccak256(abi.encode(user, maxTokenAmount, isEarlyClaim, salt))
            )
        );
    ...
    function _getClaimableState(
        ...
        claimableState.base =
            (maxTokenAmount * config.baseTokenClaimBps) /
            PERCENTAGE_BASIS_POINTS_DENOMINATOR;
        claimableState.bonus = maxTokenAmount - claimableState.base;

        if (
            config.tokenAmount == 0 ||
            (config.isRefunding && userState.claimed == 0)
        ) {
        ...
        if (userState.hasEarlyClaimed || unlockState.isBeforeUnlock)
            return claimableState;
        ...
        if (unlockState.isUnlocking) {
            ...
            claimableState.vested =
                (claimableState.bonus * unlockState.unlockElapsedDuration) /
                config.unlockDuration;
            claimableState.claimable =
                claimableState.base +
                claimableState.vested -
                claimableState.claimed;
            claimableState.earlyVestableBonus = _calcEarlyVestableBonus(
                claimableState,
                config,
                unlockState.unlockElapsedDuration
            );
        } else {
            // unlock completed
            claimableState.claimable =
    ...
    function _validateClaimParams(
        ...
        if (config.tokenAmount == 0) {
            revert IBUILDFactory.ProjectSeasonDoesNotExist(
                param.seasonId,
                address(i_token)
            );
        }
        ...
        if (
            unlockState.isUnlocking &&
            param.isEarlyClaim &&
            userState.hasEarlyClaimed
        ) {
        ...
        if (userState.claimed == 0 && config.isRefunding) {
            ...
            revert IBUILDFactory.ProjectSeasonIsRefunding(
                address(i_token),
                param.seasonId
            );
        }
    ...
    function _claim(address user, ClaimParams[] memory params) private {
        ...
        for (uint256 i = 0; i < paramsLength; ++i) {
            ...
            (
                IBUILDFactory.ProjectSeasonConfig memory config,
                uint256 unlockStartsAt
            ) = i_factory.getProjectSeasonConfig(
                    address(i_token),
                    param.seasonId
                );
            ...
            ClaimableState memory claimableState = _getClaimableState(
                config,
                globalState,
                userState,
                unlockState,
            ...
            if (
                ...
                (claimableState.claimable == 0 &&
                    claimableState.earlyVestableBonus == 0 &&
                    param.isEarlyClaim)
            ) {
                continue;
            }
            ...
            if (claimableState.claimed == 0) {
                ...
                i_factory.reduceRefundableAmount(
                    address(i_token),
                    param.seasonId,
                    param.maxTokenAmount
                );
    ...
    function _updateClaimedAmounts(
        ...
        // @audit critical A user's `claimed` amount, a `uint248`, can overflow. `toBeClaimed` is a `uint256` and can be larger than `maxTokenAmount` due to the loyalty bonus. If `userState.claimed + toBeClaimed` exceeds `type(uint248).max`, it will overflow. This allows a user to claim more than their allocated amount over multiple transactions.
